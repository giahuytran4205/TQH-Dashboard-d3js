...
\textbf{Idiom} & Pie Chart (Biểu đồ tròn) \\
\addlinespace[0.2cm]

\textbf{What} & Loại chủ nhà (Host Type): C, Tỷ trọng đóng góp (\% of count): Q \\
\addlinespace[0.2cm]

\textbf{Why} & produce (derive) $\rightarrow$ explore $\rightarrow$ summarize
\begin{itemize}[nosep, leftmargin=*, topsep=0.1cm]
    \item Derive: phân loại chủ nhà dựa trên số lượng listing sở hữu (1 phòng = Cá nhân, $>$ 1 phòng = Chuyên nghiệp)
    \item Explore và Summarize: xem nhóm nào đang chiếm ưu thế kiểm soát nguồn cung trên thị trường
\end{itemize} \\
\addlinespace[0.2cm]

\textbf{How} & \textbf{Encode:}
\begin{itemize}[nosep, leftmargin=*, topsep=0.1cm]
    \item Mark: Area (vùng góc quạt)
    \item Channel:
    \begin{itemize}[nosep, leftmargin=0.5cm]
        \item Angle: Độ lớn của tỷ trọng phần trăm (0\%--100\%)
        \item Color: Phân biệt 2 nhóm chủ nhà
    \end{itemize}
\end{itemize}
...
\textbf{Manipulate:}
\begin{itemize}[nosep, leftmargin=*, topsep=0.1cm]
    \item Hover + Tooltip: Rê chuột vào từng phần quạt để xem chi tiết tên nhóm và tỷ lệ \% chính xác
\end{itemize}
...
\begin{itemize}[nosep, leftmargin=*, topsep=0pt]
    \item Main key: 2 (Cá nhân, Chuyên nghiệp)
    \item Value range: 0\%--100\%
\end{itemize} \\
\end{longtable}
