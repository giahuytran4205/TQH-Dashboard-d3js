...
\textbf{Idiom} & Grouped Bar Chart \\
\addlinespace[0.2cm]

\textbf{What} & Borough: Categorical (Key nhóm chính) \newline
...
\begin{itemize}[nosep, leftmargin=*, topsep=0.1cm]
    \item Mark: Bar (grouped -- Stack Marks OFF)
    \item Channel:
    \begin{itemize}[nosep, leftmargin=0.5cm]
        \item Pos X: Borough $\times$ Room Type (Categorical -- tạo cụm cột cạnh nhau)
        \item Pos Y: MEDIAN(price\_per\_person) (Quantitative -- length)
        \item Hue Color: Room Type (Categorical -- 4 màu)
        \item Label: giá trị median ở đầu mỗi cột
        \item Note: Analysis $\rightarrow$ Stack Marks $\rightarrow$ Off (bắt buộc để tạo Grouped, không phải Stacked)
    \end{itemize}
\end{itemize}
...
\textbf{Selection:}
\begin{itemize}[nosep, leftmargin=*, topsep=0.1cm]
    \item Hover để xem median price/person theo từng borough $\times$ room type
\end{itemize}
...
\begin{itemize}[nosep, leftmargin=*, topsep=0.1cm]
    ...
    \item Aggregate: MEDIAN(price\_per\_person) theo borough $\times$ room type
\end{itemize} \\
\addlinespace[0.2cm]

\textbf{Why} & compare $\rightarrow$ summarize \newline
...
\begin{itemize}[nosep, leftmargin=*, topsep=0pt]
    \item Main key: 5 borough $\times$ 4 room type = 20 cột
