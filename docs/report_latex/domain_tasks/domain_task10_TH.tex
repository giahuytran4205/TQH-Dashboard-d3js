...
\textbf{Idiom} & Word Cloud (Đám mây từ vựng) \\
\addlinespace[0.2cm]

\textbf{What} & Từ khóa (Keyword): C, Tần suất xuất hiện (Count): Q \\
\addlinespace[0.2cm]

\textbf{Why} & produce (derive) $\rightarrow$ explore $\rightarrow$ summarize
\begin{itemize}[nosep, leftmargin=*, topsep=0.1cm]
    \item Dẫn xuất (derive): bóc tách văn bản loại bỏ stopwords để lấy ra các unigram/bigram (1--2 từ) có ý nghĩa.
    \item Explore: Khám phá nhanh và tóm tắt (summarize) các chủ đề / ngữ cảnh lưu trú được thảo luận nhiều nhất.
\end{itemize} \\
\addlinespace[0.2cm]

\textbf{How} & \textbf{Encode:}
\begin{itemize}[nosep, leftmargin=*, topsep=0.1cm]
    \item Mark: Text (Chữ viết)
    \item Channel:
    \begin{itemize}[nosep, leftmargin=0.5cm]
        \item Size (Kích thước chữ): Tỷ lệ thuận với tần suất đếm được.
        \item Color: Biểu diễn độ lớn của Tần suất (Q) (chữ càng đậm = xuất hiện càng nhiều).
    \end{itemize}
\end{itemize}
...
\textbf{Reduce:}
\begin{itemize}[nosep, leftmargin=*, topsep=0.1cm]
    \item Filter: Áp dụng tập stopwords mở rộng để loại bỏ các từ vô nghĩa.
\end{itemize} \\
\addlinespace[0.2cm]

\textbf{Scale} &
\begin{itemize}[nosep, leftmargin=*, topsep=0pt]
    \item Main key: Top các từ khóa xuất hiện nhiều nhất
\end{itemize} \\
\end{longtable}
