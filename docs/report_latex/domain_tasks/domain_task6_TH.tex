...
\textbf{Idiom} & Scatter Plot \\
\addlinespace[0.2cm]

\textbf{What} & Price: Quantitative (Key trục X) \newline
...
\begin{itemize}[nosep, leftmargin=*, topsep=0.1cm]
    ...
    \begin{itemize}[nosep, leftmargin=0.5cm]
        \item Pos X: Price (Quantitative)
        \item Pos Y: review\_scores\_rating (Quantitative)
        \item Hue Color: good\_deal\_flag (Categorical -- xanh=Good Deal, xám=Normal)
        \item Size: number\_of\_reviews (Quantitative)
    \end{itemize}
    \item Reference Lines:
    \begin{itemize}[nosep, leftmargin=0.5cm]
        \item Dọc (trục X): MEDIAN(price) -- phân cách giá cao/thấp
        \item Ngang (trục Y): Constant = 4.8 -- ngưỡng Good Deal Flag
    \end{itemize}
\end{itemize}
...
\textbf{Manipulate:}
\begin{itemize}[nosep, leftmargin=*, topsep=0.1cm]
    \item Hover để xem giá, rating, room\_type, neighbourhood, number\_of\_reviews
\end{itemize}

\vspace{0.2cm}
\textbf{Facet:}
\begin{itemize}[nosep, leftmargin=*, topsep=0.1cm]
    \item Superimpose: Reference Lines đặt lên scatter plot
\end{itemize}

\vspace{0.2cm}
\textbf{Reduce:}
\begin{itemize}[nosep, leftmargin=*, topsep=0.1cm]
    \item Filter: number\_of\_reviews $\geq$ 5 (loại listing ít đánh giá, không đủ tin cậy)
\end{itemize} \\
\addlinespace[0.2cm]

\textbf{Why} & produce $\rightarrow$ discover $\rightarrow$ compare \newline
...
\begin{itemize}[nosep, leftmargin=*, topsep=0pt]
    \item Color: 2 (Good Deal / Normal)
    \item Items: listing với $\geq$ 5 reviews, price\_is\_outlier = False
\end{itemize} \\
\end{longtable}
